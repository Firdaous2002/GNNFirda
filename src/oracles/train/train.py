...
class MultilabelTrainer:
    def __init__(self, cfg: DictConfig, dataset: Dataset, model: nn.Module, loss: nn.Module = None) -> None:
        ...
        self.cfg = cfg
        self.dataset = dataset
        # For multilabel, number of classes equals the number of labels (columns in y)
        self.num_classes = self.dataset.y.size(1)
        # Use BCEWithLogitsLoss (or a provided loss) for multilabel classification.
        self.loss = nn.BCEWithLogitsLoss()
        # Multilabel metric – using a simple threshold of 0.5.
        self.metric = MultilabelAccuracy()
        self.model = model
        self.optimizer = optim.Adam(self.model.parameters(), lr=cfg.trainer.lr)
        # For simplicity, assume one batch containing all nodes.
        # If you want mini-batching, you can use DataLoader.
    ...
    def _train(self, epoch: int):
        ...
        self.metric.update(predictions, self.dataset.y[self.dataset.train_mask].float())
    ...
    def _test(self):
        ...
        with torch.no_grad():
            output = self.model(self.dataset.x.to(self.device), self.dataset.edge_index.to(self.device))
            loss_test = self.loss(output[self.dataset.test_mask], self.dataset.y[self.dataset.test_mask].float())
            predictions = (torch.sigmoid(output[self.dataset.test_mask]) > 0.5).float()
            self.metric.update(predictions, self.dataset.y[self.dataset.test_mask].float())
    ...
    def start_training(self):
        ...
        for epoch in range(self.cfg.trainer.epochs):
            train_loss, train_accuracy = self._train(epoch=epoch)
            test_loss, test_accuracy = self._test()
            print(f"Epoch: {epoch:4d} Train Loss: {train_loss:.4f} Train Acc: {train_accuracy:.4f} " +
        ...
        model_path = f"data/models/{self.cfg.dataset.name}_{self.cfg.model.name}_{self.cfg.model.hidden_layers}_epochs_{self.cfg.trainer.epochs}.pt"
        torch.save(self.model.state_dict(), model_path)
        metrics.to_csv(model_path.replace('.pt', '_METRICS.csv'))