...
def set_run_name(cfg, run):
    ...
    run_name: str = f"{cfg.task.name}_{name}_{cfg.dataset.name}_{cfg.model.name}_{cfg.optimizer.name}_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
...
def train(cfg):    
    ...
    with wandb.init(project=cfg.logger.project, group="experiment_1", mode=cfg.logger.mode) as run:
        ...
        from src.oracles.train.train import Trainer, GraphTrainer, MultilabelTrainer
        ...
        print(torch.sum(dataset.y, dim=0))
        ...
        print('numero_di_classioracle', datainfo.num_classes)
        print('data_info', datainfo)
        oracle = oracle(
